    ...
    while true do
        ...
        if housesInfo then
            ...
            for k, v in pairs(housesInfo) do
                ...
                local houseExit = vec3(houseData.enter.x + houseData.exit.x, houseData.enter.y + houseData.exit.y,
                    houseData.enter.z + houseData.exit.z)
                ...
                if distance < 3 then
                    ...
                    if houseData.owner == 'The State' then
                        ...
                        DrawText3D(houseEnter.x, houseEnter.y, houseEnter.z,
                            houseData.tag ..
                            "~y~" ..
                            houseData.name ..
                            "~n~~w~Tipul casei: " .. houseData.type .. "~n~Pret: $" .. FormatNumber(houseData.price))
                        DrawText3D(houseEnter.x, houseEnter.y, houseEnter.z - 0.55, '~y~(~w~/buyhouse~y~)')
                        DrawMarker(0, houseEnter.x, houseEnter.y, houseEnter.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 209,
                            82, 255, true, false, false, true, false, false, false)
                        closestHouse = v
                    else
                        if not houseData.locked then
                            DrawMarker(0, houseEnter.x, houseEnter.y, houseEnter.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 0,
                                255, 0, 255, true, false, false, true, false, false, false)
                        else
                            DrawMarker(0, houseEnter.x, houseEnter.y, houseEnter.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255,
                                0, 0, 255, true, false, false, true, false, false, false)
                        end
                        DrawText3D(houseEnter.x, houseEnter.y, houseEnter.z,
                            "~r~" ..
                            houseData.name ..
                            ...
                            "Owner: " ..
                            houseData.owner ..
                            "" .. "~n~Rent: $" .. FormatNumber(houseData.rent) .. "~n~Tenants: " ..
                            #houseData.tenants .. "")
                        closestHouse = v
                    end
                end
                if distance2 < 3 then
                    ...
                    if houseData.owner ~= 'The State' or houseData.ownerId ~= 0 then
                        DrawText3D(houseExit.x, houseExit.y, houseExit.z,
                            "Casa lui ~r~" .. houseData.owner .. "~n~~w~~r~(~w~/exit~r~)")
                    else
                        DrawText3D(houseExit.x, houseExit.y, houseExit.z, "Casa nedetinuta~n~~w~~r~(~w~/exit~r~)")
                    end
                    if not houseData.locked then
                        DrawMarker(0, houseExit.x, houseExit.y, houseExit.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 0, 255, 0,
                            255, true, false, false, true, false, false, false)
                    else
                        DrawMarker(0, houseExit.x, houseExit.y, houseExit.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 0, 0,
                            255, true, false, false, true, false, false, false)
                    end
                    closestHouse = v
                end
                ...
                if not isTenant then
                    if PlayerData.rentedHouse == 0 or not PlayerData.rentedHouse then
                        if PlayerData.cash >= hData.rent then
                            ...
                            sendNotification("Casa",
                                'Ai devenit chirias la casa cu numarul ' ..
                                closestHouse.id .. ' si ai platit $' .. FormatNumber(hData.rent) .. '!', "success")
                        else
...
function EnterHouse(house)
    ...
    --print
    local shellData = CreateShell(housesTypes[houseData.type],
        vec3(houseData.enter.x, houseData.enter.y, houseData.enter.z - 20.0))

    --print

    local pCoords = GetEntityCoords(PlayerPedId())
    local newCoords = vec3(houseData.enter.x + housesTypes[houseData.type].exit.x,
        houseData.enter.y + housesTypes[houseData.type].exit.y, houseData.enter.z + housesTypes[houseData.type].exit.z)
    ...
    if tonumber(response) then
        ...
        Core.TriggerCallback("Players:IsOnline", function(isOnline)
            ...
            if isOnline then
                ...
                ShowDialog('Vinde casa', "Scrie suma cu care vrei sa vinzi casa jucatorului respectiv.",
                    'Houses:SellToPlayer', true, true, "client")
            else
...
RegisterNetEvent("Houses:HandleCheck", function(response)
    if response:lower() == 'da' then
        ...
        tosellHouse = false
        sendNotification("Ai vandut casa", 'Ai primit $' .. FormatNumber(math.floor(sell)) .. ' din vanzarea casei.',
            "success")
        return
    else
...
local housesMenu = MenuV:CreateMenu(false, 'Casele tale', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv',
    'houses-menu')
local houseLoadingMenu = MenuV:CreateMenu(false, 'Incarcam casele', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv',
    'house-loading-menu')
local houseMenu = MenuV:CreateMenu(false, 'Casa', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv', 'house-menu')
local chiriasiMenu = MenuV:CreateMenu(false, 'Chiriasi', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv',
    'renters-menu')
local chiriasMenu = MenuV:CreateMenu(false, 'Chiriasi', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv',
    'renter-menu')
local sellHousemenu = MenuV:CreateMenu(false, 'Vinde casa', 'centerright', 0, 0, 0, 'size-125', 'none', 'menuv',
    'sellhouse-menu')
        ...
        for k, v in pairs(houses) do
            ...
            house:On('select', function()
                ...
                setCp:On('select', function()
                    --print
                    CreateCP(1, vec3(hData.enter.x, hData.enter.y, hData.enter.z), { 255, 0, 0, 255 }, 1.0, 25,
                        function()
                        end)
                end)
                ...
                changeRent:On("select", function()
                    tosellHouse = v
                    ShowDialog("Schimba pretul chiriei", "Scrie mai jos pretul pe care vrei sa-l plateasca chiriasii.",
                        "Houses:ChangeRentHandle", true, true, "client")
                    MenuV:CloseAll()
                end)
                    ...
                    Core.TriggerCallback("Houses:IsOwner", function(isOwner)
                        if isOwner then
                            ...
                            sellHousemenu:AddButton({
                                ...
                                ShowDialog("Vinzi casa",
                                    "Esti sigur ca vrei sa vinzi aceasta casa pentru $" ..
                                    FormatNumber(math.floor(sellAmount)) .. "? Scrie DA mai jos daca esti deacord.",
                                    "Houses:HandleCheck", true, true, "client")
                            end)
                            ...
                            sellHousemenu:AddButton({
                                ...
                                tosellHouse = v
                                ShowDialog("Vinzi casa", "Scrie mai jos ID-ul jucatorului caruia vrei sa-i vinzi casa.",
                                    "Houses:HandleSellToPlayer", true, true, "client")
                            end)
...
RegisterCommand("createhouse", function(source, args, rawCommand)
    ...
    local houseData = {
        ...
        location = GetNameOfZone(pCoords.x, pCoords.y, pCoords.z) ..
        " - " .. GetStreetNameFromHashKey(GetStreetNameAtCoord(pCoords.x, pCoords.y, pCoords.z)),
        tenants = {},
        rent = 125000,
        ownerId = 0,
        owner = 'The State'
    }
    sendNotification("Casa", 'Ai creat o casa de tip: ' .. house_type ..
    ' cu pretul de: $' .. FormatNumber(house_price) .. '!', "success")
...
RegisterNetEvent('Shell:Create', function(shell)
    ...
    local shellData = CreateShell(housesTypes[shell],
        vec3(createShellCoords.x, createShellCoords.y, createShellCoords.z - 20.0))
